...
maphelper.switchMap = function (mapType) {
	if (maphelper.map == null || maphelper.commonmap == null || maphelper.administrativemap == null) return;
	switch (mapType) {
		case 'satellite':
...
maphelper.getLayerSld = function (layername, propertyName, propertyValue, strokeColor, fillColor) {
	var sld = '<?xml version="1.0" encoding="UTF-8"?>' + '<sld:StyledLayerDescriptor xmlns="http://www.opengis.net/sld" xmlns:sld="http://www.opengis.net/sld" xmlns:ogc="http://www.opengis.net/ogc" xmlns:gml="http://www.opengis.net/gml" version="1.0.0">' + '<sld:UserLayer>' + '<sld:LayerFeatureConstraints>' + '<sld:FeatureTypeConstraint/>' + '</sld:LayerFeatureConstraints>' + '<sld:Name>' + layername + '</sld:Name>' + '<sld:UserStyle>' + '<sld:Title/>' + '<sld:FeatureTypeStyle>' + '<sld:Name>group 0</sld:Name>' + '<sld:FeatureTypeName>Feature</sld:FeatureTypeName>' + '<sld:SemanticTypeIdentifier>generic:geometry</sld:SemanticTypeIdentifier>' + '<sld:SemanticTypeIdentifier>simple</sld:SemanticTypeIdentifier>' + '<sld:Rule>' + '<sld:Name>New Rule</sld:Name>' + '<ogc:Filter>' + '<ogc:Or>' + '<ogc:PropertyIsEqualTo>' + '<ogc:PropertyName>' + propertyName + '</ogc:PropertyName>' + '<ogc:Literal>' + propertyValue + '</ogc:Literal>' + '</ogc:PropertyIsEqualTo>' + '</ogc:Or>' + '</ogc:Filter>' + '<sld:PolygonSymbolizer>' + '<sld:Fill>' + '<sld:CssParameter name="fill">' + fillColor + '</sld:CssParameter>' + '<sld:CssParameter name="fill-opacity">0.3</sld:CssParameter>' + '</sld:Fill>' + '<sld:Stroke>' + '<sld:CssParameter name="stroke">' + strokeColor + '</sld:CssParameter>' + '<sld:CssParameter name="stroke-width">1</sld:CssParameter>' + '<sld:CssParameter name="stroke-opacity">0.5</sld:CssParameter>' + '</sld:Stroke>' + '</sld:PolygonSymbolizer>' + '</sld:Rule>' + '</sld:FeatureTypeStyle>' + '</sld:UserStyle>' + '</sld:UserLayer>' + '</sld:StyledLayerDescriptor>';
	return sld;
};
...
staPoint.initSKImg = function () {
	staPoint.img = _maphelper.maphelper.addImage("./images/me_noProduct.png", 72.4, 15.5, 136.5, 54.5, 0);
};
...
staPoint.drawArea = function (km, lat, lon) {
	...
	staPoint.bcircle = L.circle(latlng, km, style1).addTo(_maphelper.maphelper.map);
	staPoint.scircle = L.circle(latlng, 60, style2).addTo(_maphelper.maphelper.map);
	staPoint.mapDiv = _maphelper.maphelper.addDiv(lon + 0.8, lat, "<div style='color:#ff0000;'>80km</div>");
		...
		success: function success(data) {
			...
			if (data.length > 0) {
				if (staPoint.img) {
					...
					staPoint.img = _maphelper.maphelper.addImage(data[data.length - 1].url, 72.4, 15.5, 136.5, 54.5, 0.8);
				}
			} else {
				alert("没有获取到当前时间的色斑图");
			}
	...
	$.ajax({
		...
		success: function success(data) {
			...
			if (staPoint.img) {
				...
				staPoint.img = _maphelper.maphelper.addImage("http://10.1.64.154/pic/" + data[data.length - 1].url, 72.4, 15.5, 136.5, 54.5, 0.8);
			}
		}
	});
};
...
staPoint.prcsData = function (result, flag, funitemmenuid, lon, lat) {
	...
	if (totalPageNum != 0) {
		for (var _i = 0; _i < totalPageNum; _i++) {
			var tempArrPageRecord = syntheical.groupRecord(publicPageSize * _i, publicPageSize * (_i + 1), list);
...
staPoint.prcsData2 = function (result, flag, funitemmenuid, lon, lat) {
	...
	if (totalPageNum != 0) {
		for (var _i3 = 0; _i3 < totalPageNum; _i3++) {
			var tempArrPageRecord = syntheical.groupRecord(publicPageSize * _i3, publicPageSize * (_i3 + 1), list);
		...
		if (flag === 0) {
			...
			if (globalParam.provinceFocus) {
				for (var _i4 = 0; _i4 < provinceList.length; _i4++) {
					if (provinceList[_i4].provincecode === globalParam.provinceFocusId) {
			...
			var str = "<div class='searchBox'><div class='me_search'><input type='text' class='staNo' placeholder='请输入台站号或台站名'/><span class='searchBtn'></span></div></div>";
	...
	if (tempArr != undefined) {
		for (var i = 0; i < tempArr.length; i++) {
			if (tempArr[i] == undefined || tempArr[i] == null) {
				...
				tbody += "<tr class='me_warningInfoTr' " + "Station_levl='" + tempArr[i][arrEleval[5]] + "'" + " staId='" + tempArr[i][arrEleval[1]] + "' staName='" + tempArr[i][arrEleval[0]] + "' lon='" + tempArr[i][arrEleval[3]] + "' lat='" + tempArr[i][arrEleval[2]] + "' province='" + tempArr[i][arrEleval[4]] + "' funid ='" + tempArr[i]["funid"] + "'>";
				...
				tbody += "<td>" + serNo + "</td><td title='" + areaTi + "'>" + area_ + "</td><td title='" + stationTi + "'>" + station_ + "(" + staNo_ + ")" + "</td><td>" + value + "</td>";
				tbody += "</tr>";
			}
		}
...
staPoint.addLocationMark = function (lat, lon) {
	...
	staPoint.removeLocationMark = _maphelper.maphelper.addClickableMarker2(common.translateNum2(lon), common.translateNum2(lat), iconUrl, null, null);
};
...
staPoint.drawPoint = function (result, funitemmenuid, splitList) {
	...
	if (length > 0) {
		...
		for (var i = 0; i < length; i++) {
			...
			_echartsData.push([list[i][arrEle[3]], list[i][arrEle[2]], obj1.value, obj1]);
		}
	}
	...
	try {
		...
		if (task == null) {
			task = setTimeout(function () {
				staPoint.overlay.setOption(options);task = null;
			}, 150);
		} else {
			clearTimeout(task);
			task = setTimeout(function () {
...
staPoint.drawPoint2 = function (result, funitemmenuid, splitList) {
	...
	if (length > 0) {
		...
		for (var _i5 = 0; _i5 < length; _i5++) {
			...
			geoCoordMap[obj1.name] = obj2;
			_echartsData2.push([list[_i5][arrEle[3]], list[_i5][arrEle[2]], obj1.value, obj1]);
		}
	}
	if (_maphelper.maphelper.map.getZoom() >= 12) {
		var _isShow3 = true;
	...
	$.ajax({
		...
		success: function success(result) {
			...
			if (result.returnCode === "0") {
				...
				} else if (globalParam.staid == '1150101020' || globalParam.staid == '1150101021' || globalParam.staid == '1150101022' || globalParam.staid == '1150101023' || globalParam.staid == '1150101024') {
					//降水
					emgcy.getWarningCountyData(areaid, '11B03,11B20,', Cnty);
				} else if (globalParam.staid == '115990104' || globalParam.staid == '115990108' || globalParam.staid == '115990109' || globalParam.staid == '115990110' || globalParam.staid == '115990111') {
					//风
				...
				if (vis != undefined && vis != "" && vis != "999999") {
					$(".detail ul").append("<li>小时能见度:" + vis / 1000 + "km</li>");
				} else {
...
staPoint.drawSamePeriodtem = function (index, title) {
	...
	syntheical.chart = $("#me_timeCompare").highcharts('StockChart', {
		...
		plotOptions: {
			column: {
				dataGrouping: {
					units: [['day', [1]], ['week', [1]], ['month', [1, 3, 6]], ['year', null]]
				}
			}
		},
		credits: {
...
staPoint.clickDetail = function (funitemmenuid, station, subtitle, lon, lat, stationLevel) {
	...
	for (var _i6 = 0; _i6 < hovers.length; _i6++) {
		...
		if (resultsk != undefined && resultyb != undefined) {
			resultall[array3[_i6]] = common.mergeArray(resultsk[_i6], resultyb[_i6]);
		...
		var _loop = function _loop(_i8) {
			$.each(emgcy.arrWarning, function (j, n) {
				if (n.time == skCategories[_i8]) {
					var _url = ctxStatic + '/ultra/img/disasterWarning/' + n.signaltypecode + "_" + n.signallevelcode + ".png";
					var data = skData[_i8];
					skData[_i8] = {
						y: data,
						marker: {
		...
		tooltip: {
			formatter: function formatter() {
				if (globalParam.staid == "115010201") {
					...
					var html = year + "年" + month + "月" + day + "日" + hour + "时：" + this.y + resultall["units" + index];
					for (var _i9 = 0; _i9 < emgcy.arrWarning.length; _i9++) {
						if (strTime == emgcy.arrWarning[_i9].time) {
							html += "  ";
							if (emgcy.arrWarning[_i9].codename != undefined && emgcy.arrWarning[_i9].codename != null && emgcy.arrWarning[_i9].codename != "") {
								html += emgcy.arrWarning[_i9].codename;
							}
							if (emgcy.arrWarning[_i9].signallevel != undefined && emgcy.arrWarning[_i9].signallevel != null && emgcy.arrWarning[_i9].signallevel != "") {
...
staPoint.getTimesList = function (times, type, nums) {
	...
	if (len >= 10) {
		...
		var strFormat = strOfTime.substring(0, 4) + "-" + strOfTime.substring(4, 6) + "-" + strOfTime.substring(6, 8) + " " + strOfTime.substring(8, 10) + "时";
		var strFormat2 = strOfTime.substring(0, 4) + "-" + strOfTime.substring(4, 6) + "-" + strOfTime.substring(6, 8) + " " + strOfTime.substring(8, 10) + ":" + strOfTime.substring(10, 12) + ":" + strOfTime.substring(12, 14);
...
common.canvasPositionReset = function () {
	var lonlat = _maphelper.maphelper.map.containerPointToLatLng(L.point(0, 0)); // 左上角
	var topLeft = _maphelper.maphelper.map.latLngToLayerPoint([lonlat.lat, lonlat.lng]);
	...
	$.ajax({
		...
		success: function success(result) {
			...
			for (var i = 0; i < list.length; i++) {
				strHtml += "<div style='width:" + width + ";height:15px;background-color:" + list[i].color + ";float:left;'></div>";
			}
			strHtml += "</div>";
			strHtml += "<div style='overflow:hidden;'>";
			if (list.length < 18) {
				for (var i = 0; i < list.length; i++) {
					...
					strHtml += "<div style='width:" + width + ";float:left;font-size:12px;text-align:right;'>" + name + "</div>";
				}
			} else {
				for (var i = 0; i < list.length; i = i + 2) {
					var name = list[i].name;
					strHtml += "<div style='width:" + 2 * parseInt(width) + "px;float:left;font-size:12px;text-align:right;'>" + name + "</div>";
				}
			}
			strHtml += "</div></div>";
...
common.drawPoint = function (myctx, lon, lat, value) {
	var point = _maphelper.maphelper.map.latLngToContainerPoint(new L.LatLng(lat, lon));
	myctx.beginPath();
	myctx.lineWidth = 4.5;
...
common.addWarnMarker = function (data, arrMarker) {
	...
	var m = _maphelper.maphelper.addClickableMarker(common.translateNum(data.lon), common.translateNum(data.lat), {
		url: url,
		w: w,
		h: h,
		zIndexOffset: time
  	...
  	$(".staBtn").removeClass("staChecked");
		...
		if ($("#sk").hasClass("sel")) {
			...
			if (globalParam.provinceFocusId != null && globalParam.provinceFocusId != "" && globalParam.provinceFocusId != "1000") {
				_stationPoint2.default.getPoint2(funitemmenuid, _stationPoint2.default.datetime, provinceCode, "1");
			} else {
				_stationPoint2.default.getPoint(funitemmenuid, _stationPoint2.default.datetime, provinceCode, "1");
			}
		}
	}, function () {
		// 鼠标悬停事件
		var title = data.name + "发布" + data.codename + data.signallevel + "预警信号";
		var values = "<div class='warnInfoTip'><div class='warnInfoTipTi'>" + "<span class='warnTi'>" + title + "</span></div>" + "<div class='warnInfoTipCont'>" + data.issuecontent + "</div></div>";
		return values;
	});
	arrMarker.push(m);
};
...
common.addDisasterMarker = function (data, arrMarker) {
	...
	var m = _maphelper.maphelper.addClickableMarker(common.translateNum(data.lon), common.translateNum(data.lat), {
		...
		var values = "<div class='warnInfoTip'><div class='warnInfoTipTi'>" + "<span class='warnTi'>" + title + "</span></div>" + "<div class='warnInfoTipCont'>" + data.IMPACT + "</div></div>";
		return values;
	});
	arrMarker.push(m);
};
...
common.existWarningType = ["11B01", "11B03", "11B04", "11B05", "11B06", "11B07", "11B09", "11B11", "11B14", "11B15", "11B16", "11B17", "11B19", "11B20", "11B21", "11B22", "11B25", "11B99"];
common.warningTypeDisabled = {};
common.existDisasterType = ["GALDR", "THDDR", "CWDR", "DGDR", "HLDR", "CDDR", "ICFDR", "OMDR", "TYODR", "FOGDR", "SNODR", "STSUDR", "ERDR", "SSDR", "DWDR", "MGDDR", "HWDR", "LCDDR", "AIPDR", "RSDR", "FIRDR"];
...
common.emergency = function (data) {
	...
	if (lonlat != undefined) {
		...
		_maphelper.maphelper.addDivOfZIndex(common.translateNum(lon), common.translateNum(lat), "<div style='width:100px;font:bold 15px Microsoft Yahei;color:" + emgyColor + "'>" + data.type + data.level + "</div>", null, parseInt(zIndex), null, function () {
...
var _colormap = __webpack_require__(6);

var _windutil = __webpack_require__(7);
...
$(function () {
  ...
  // 图层工具事件处理
  $("#mapSwitch,#mapSwitchTooltip").hover(function (e) {
...
$(".menuList li").on("click", function () {
  if (!$(this).hasClass("lsel")) {
    ...
    if (globalParam.staid === "115990104") {
      ...
      _stationPoint2.default.getLastTime(globalParam.staid, "JSON_CHN", 1, 0);
      (0, _windutil.windyshow)(globalParam.staid, _stationPoint2.default.datetime);
    } else {
      _stationPoint2.default.getLastTime(globalParam.staid, "Img", 1, 0);
      (0, _colormap.getColorMap)(_stationPoint2.default.productCode, _stationPoint2.default.datetime);
      (0, _colormap.getLegend)(globalParam.staid, "Gis", 2);
      $(".lgd_sk").show();
      ...
      if (globalParam.clickFlag) {
        if (globalParam.clickFlag) {
          ...
          _stationPoint2.default.clearEchartPoint();
          _stationPoint2.default.getAreaSta(globalParam.skId, globalParam.lngOf80km, globalParam.latOf80km, _stationPoint2.default.datetime, true);
        }
        if (_stationPoint2.default.currPoint != null) {
          var station = _stationPoint2.default.currPoint;
          _stationPoint2.default.getStationDetail(globalParam.staid, station.staId, station.staName, station.lon, station.lat, station.lev);
        }
      } else {
        if ($(".staBtn").hasClass("staChecked")) {
          if (_stationPoint2.default.bcircle != null) {
          ...
          $(".skright").hide();
          if (globalParam.provinceFocus) {
            _stationPoint2.default.getPoint2(globalParam.staid, _stationPoint2.default.datetime, globalParam.provinceFocusId, "1");
          } else {
            _stationPoint2.default.getPoint2(globalParam.staid, _stationPoint2.default.datetime, "1000");
          }
        }
      }
    }
    ...
    var option = {
        ...
        series: [{
            ...
            symbol: function symbol(val, param) {
                ...
                var url = originurl + ctxStatic + "/ultras/img/atdt/" + imgName + ".png";
                return "image://" + url;
            },
            tooltip: {
                trigger: 'item',
 ...
 */
var EchartsLayer = exports.EchartsLayer = (L.version < "1.0" ? L.Class : L.Layer).extend({
    ...
    success: function success(data) {
      if (data.returnCode === "0") {
        ...
        if (_stationPoint2.default.img) {
          ...
          _stationPoint2.default.img = _maphelper.maphelper.addImage(obj.url, 72.4, 15.5, 136.5, 54.5, 0.8);
        }
      }
    }
  });
...
function appendLegend(funItemMenuId, result, flag) {
  ...
  var strHtml = "<div class='lgd_top'><span class='lgd_name'>" + result.name + "(" + result.unit + ")" + "</span>";
  if (flag === 1) {
    //重新赋值
    strHtml = "<div class='lgd_top'><span class='lgd_name'>融合分析</span></div>";
    strHtml += "<div class='smallTitle'><span style='margin-left: 10px;'>" + result.name + "(" + result.unit + ")</span><span class='lgd_time' id='lgd_fxtime'></span></div>";
  } else if (flag === 2) {
    //重新赋值
    ...
    strHtml += "<div class='smallTitle'><span style='margin-left: 10px;'>" + result.name + "(" + result.unit + ")</span><span class='lgd_time' >" + time + "</span></div>";
  } else if (flag === 3) {
    //重新赋值
    ...
    var time2 = new Date(_time.substring(0, 4), _time.substring(4, 6) - 1, _time.substring(6, 8), _time.substring(8, 10));
    ...
    if (_time == undefined || _time === "") {
      ...
      //time = time.substring(0,4)+ "-" + time.substring(4,6)+ "-" + time.substring(6,8)+ " " + time.substring(8,10) + "时";
      time3 = _time.substring(4, 6) + "月" + _time.substring(6, 8) + "日" + _time.substring(8, 10) + "时";
      time4 = timestr.substring(4, 6) + "月" + timestr.substring(6, 8) + "日" + timestr.substring(8, 10) + "时";
    ...
    strHtml += "<div class='smallTitle'><span style='margin-left: 10px;'>" + result.name + "(" + result.unit + ")</span><span class='lgd_time' >" + time3 + "—" + time4 + "</span></div>";
  }
  strHtml += "<div class='lgd_bottom'>";
  strHtml += "<div class='lgd_color'>";
  for (var i = 0; i < list.length; i++) {
    strHtml += "<div style='width:" + width + ";height:12px;background-color:" + list[i].color + ";float:left;'></div>";
  }
  strHtml += "</div>";
  ...
  if (list.length < 16) {
    if (flag === 1 && funItemMenuId === "1151302041") {
      for (var _i = 0; _i < list.length; _i++) {
        ...
        strHtml += "<div style='width:" + width + ";float:left;font-size:12px;text-align:right;'>" + (name / 100).toFixed(2) + "</div>";
      }
    } else {
      for (var _i2 = 0; _i2 < list.length; _i2++) {
        var _name = list[_i2].name;
        strHtml += "<div style='width:" + width + ";float:left;font-size:12px;text-align:right;'>" + _name + "</div>";
      }
    }
  } else {
    if (flag === 1 && funItemMenuId === "1151302041") {
      for (var _i3 = 1; _i3 < list.length; _i3 = _i3 + 4) {
        ...
        strHtml += "<div style='width:" + 4 * parseInt(width) + "px;float:left;font-size:12px;text-align:right;'>" + (_name2 / 100).toFixed(2) + "</div>";
      }
    } else {
      for (var _i4 = 2; _i4 < list.length; _i4 = _i4 + 3) {
        var _name3 = list[_i4].name;
        strHtml += "<div style='width:" + 3 * parseInt(width) + "px;float:left;font-size:12px;text-align:right;'>" + _name3 + "</div>";
      }
    }
  }
  strHtml += "</div></div>";
...
function windyshow(funitemmenuid, dateTime, province, typeCode) {
  ...
  var url = ctx + "/gis/getJson?funItemMenuId=115990104&dateTime=" + dateTime + "&position=JSON_CHN";
  removewind();
  formateWind(url);
}
...
function formateWind(url) {
  $.ajax({
    ...
    success: function success(data) {
      ...
      $.ajax({
        url: ctx + "/multiExhibition/getLegend?funItemMenuId=115990104&position=Gis",
        type: "get",
        dataType: "json",
        success: function success(result) {
...
function removewind() {
  ...
  if (_stationPoint2.default.img) {
    _stationPoint2.default.img.changeUrl(ctx + "/ultra/img/gis/me_noProduct.png", 0);
  }
}
